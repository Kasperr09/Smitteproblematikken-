...
#
# Metode:
# 1) Observerede år beregnes som simpelt gennemsnit pr. år
# 2) Outlier i 2020 (1.520,75 kr./m²) vises særskilt
#    og indgår ikke i fremskrivningsgrundlaget
# 3) Samlet gennemsnit beregnes på baggrund af observerede år
#    ekskl. outlier
# 4) Fremskrivning 2023-2026 starter fra dette gennemsnit
...
# ── 7. OBSERVEREDE ÅRSGENNEMSNIT ────────────────────────────

observeret_okb <- Omkostningsbestemt_leje_beregning %>%
  group_by(År) %>%
  summarise(
    Antal_obs = n(),
    Gns_leje_pr_m2 = mean(Leje_pr_m2, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(År) %>%
  mutate(
...
# ── 8. SAMLET GENNEMSNIT UDEN OUTLIER ───────────────────────

samlet_gns <- mean(observeret_okb$Gns_leje_pr_m2, na.rm = TRUE)

cat("Samlet gennemsnit uden outlier:", round(samlet_gns, 2), "\n")
...
# ── 11. FREMSKRIVNING FRA SAMLET GENNEMSNIT ─────────────────
...
fremskrivning_okb$Gns_leje_pr_m2[1] <- samlet_gns * (1 + v2023 / 100)
fremskrivning_okb$Gns_leje_pr_m2[2] <- fremskrivning_okb$Gns_leje_pr_m2[1] * (1 + v2024 / 100)
fremskrivning_okb$Gns_leje_pr_m2[3] <- fremskrivning_okb$Gns_leje_pr_m2[2] * (1 + v2025 / 100)
fremskrivning_okb$Gns_leje_pr_m2[4] <- fremskrivning_okb$Gns_leje_pr_m2[3] * (1 + v2025 / 100)
...
linje_obs_okb <- observeret_vis %>%
  select(År, Gns_leje_pr_m2)

linje_fremsk_okb <- bind_rows(
  tibble(
    År = 2022,
    Gns_leje_pr_m2 = samlet_gns
  ),
  fremskrevet_vis %>%
    select(År, Gns_leje_pr_m2)
)
...
y_top <- max(
  observeret_vis$Gns_leje_pr_m2,
  fremskrevet_vis$Gns_leje_pr_m2,
...
p_okb_fremskriv <- ggplot() +
  ...
  annotate(
    ...
    ymin = 0,
    ymax = max(observeret_vis$Gns_leje_pr_m2, na.rm = TRUE) * 0.98,
    fill = "#D9DEE2",
    alpha = 0.35
  ) +
  ...
  annotate(
    ...
    label = paste0(
      "Gns: ",
      format(round(samlet_gns, 0), big.mark = ".", scientific = FALSE),
      " kr."
    ),
    hjust = 0,
    vjust = -0.55,
  ...
  labs(
    ...
    subtitle = paste(
      "2016-2022 er observerede år. 2023-2026 er fremskrevet fra samlet gennemsnit uden outlier.",
      "Outlieren i 2020 vises særskilt og indgår ikke i fremskrivningen."
    ),
    x = "År for lejefastsættelse",
    y = "Gns. godkendt leje pr. m2 (kr./år)",
    caption = paste(
      ...
      "Den røde stiplede linje viser samlet gennemsnit uden outlier.",
      "Fremskrivningen tager udgangspunkt i gennemsnittet og ikke i den observerede 2022-værdi."
    )
  ) +
...
ggsave(
  filename = "omkostningsbestemt_leje_fremskrivning_HUS3.pdf",
