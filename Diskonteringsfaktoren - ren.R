...
df <- df %>%
  ...
  mutate(
    ...
    label = paste0(number(vaerdi, accuracy = 0.01, decimal.mark = ","), " %")
  )
...
p <- ggplot(df, aes(x = x_pos, y = vaerdi, fill = komponent)) +
  ...
  labs(
    ...
    subtitle = "Build-up-model baseret på risikofri rente og selvstændige risikopræmier",
    y = "Årligt afkast (%)",
    x = NULL,
    fill = NULL,
    caption = "Note: Figuren viser diskonteringsrenten uden indregning af reguleringsrisiko."
  ) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 12, colour = "#4F4F4F"),
...
# -----------------------------
ggsave(
  filename = "diskonteringsrente_uden_reguleringsrisiko.png",
  plot = p,
  width = 11,
  height = 8,
  dpi = 320,
